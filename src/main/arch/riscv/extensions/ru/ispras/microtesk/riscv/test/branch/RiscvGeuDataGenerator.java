...
import ru.ispras.testbase.knowledge.branch.bgeu.BgeuThenElseGenerator;
import ru.ispras.testbase.knowledge.integer.IntNumber;
import ru.ispras.testbase.knowledge.iterator.Iterator;

/**
 * {@link RiscvGeuDataGenerator} is a test data generator for BGEU instructions.
 *
...
public final class RiscvGeuDataGenerator extends RiscvBranchDataGenerator {
  ...
  public Iterator<TestData> generateThen(final TestBaseQuery query) {
    ...
    return getTestData(query, BgeuThenElseGenerator.generateOperandsThen(operands));
  }

  @Override
  public Iterator<TestData> generateElse(final TestBaseQuery query) {
    final IntNumber[] operands = getOperands(query, 2);
    return getTestData(query, BgeuThenElseGenerator.generateOperandsElse(operands));
  }
}
